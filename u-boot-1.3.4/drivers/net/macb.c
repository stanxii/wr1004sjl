...
static u16 macb_smi_read(struct macb_device *macb, u16 phy, u8 reg)
{
	unsigned long netctl;
	unsigned long netstat;
	unsigned long frame;
...
static int macb_phy_init(struct macb_device *macb)
{
	...
	phy_id = macb_mdio_read(macb, MV88E6171R_PORT_REG_PID);
	if (phy_id == 0xffff) {
	...
	{
		printf("%s: Detect PHY ID: 0x%X\n", netdev->name, (phy_id>>4)&0xFFF);		
		ncfgr = macb_readl(macb, NCFGR);
		//printf("macb_readl NCFGR: %x\n", ncfgr);
		ncfgr &= ~(MACB_BIT(SPD) | MACB_BIT(FD));
		ncfgr |= MACB_BIT(SPD);
		ncfgr |= MACB_BIT(FD);
		macb_writel(macb, NCFGR, ncfgr);
		//printf("macb_writel NCFGR: %x\n", ncfgr);
		return 1;		
	}
