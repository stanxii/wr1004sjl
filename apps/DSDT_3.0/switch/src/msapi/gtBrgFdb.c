...
)
{
    ...
    *found = GT_FALSE;
    gtMemCpy(entry.macAddr.arEther,atuEntry->macAddr.arEther,6);
    entry.DBNum = atuEntry->DBNum;
/*printk("@@@@@@@@@@@@@ gfdbFindAtuMacEntry  dnNum %d\n", entry.DBNum); */
...
)
{
    ...
    if(atuOp == SERVICE_VIOLATIONS)
    {
...
#ifdef GT_RMGMT_ACCESS
        {
          ...
          for(i = 0; i < 3; i++)
          {
            entry->macAddr.arEther[2*i] = regAccess.rw_reg_list[i].data >> 8;
            entry->macAddr.arEther[1 + 2*i] = regAccess.rw_reg_list[i].data & 0xFF;
          }
        }
#else
        for(i = 0; i < 3; i++)
    ...
    if(atuOp == GET_NEXT_ENTRY)
    {
...
#ifdef GT_RMGMT_ACCESS
        {
          ...
          for(i = 0; i < 3; i++)
          {
            entry->macAddr.arEther[2*i] = regAccess.rw_reg_list[i+1].data >> 8;
            entry->macAddr.arEther[1 + 2*i] = regAccess.rw_reg_list[i+1].data & 0xFF;
          }
          data = regAccess.rw_reg_list[4].data;
        }
#else
        data = 1;
        while(data == 1)
