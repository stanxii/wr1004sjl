...
/*int msg_reg_mmead_get_userhfid(uint8_t ODA[], T_MMEAD_CNU_INFO *activeCnu)
{
	uint32_t buf[MAX_UDP_SIZE] = {0};
	T_Msg_Header_MMEAD h;
	T_REQ_Msg_MMEAD *r = (T_REQ_Msg_MMEAD *)buf;
	h.M_TYPE = 0xCC08;
	h.DEV_TYPE = WEC_3801I;
	...
	while( CMM_SUCCESS == __msg_reg_mmead_communicate(buf, sizeof(h)))
	{
		memcpy((void *)activeCnu->userhfid, (void *)(r->BUF),sizeof(uint32_t));
		return CMM_SUCCESS;
	}
	return CMM_SUCCESS;
}
