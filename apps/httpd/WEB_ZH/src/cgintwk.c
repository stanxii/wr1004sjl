...
void cgiCnuProfileExt(char *query, FILE *fs)
{
    ...
    fprintf(fs, "        $('#cpuvid').removeAttr('disabled');\n");
	fprintf(fs, "        $('#eth1vmode').removeAttr('disabled');\n"); 
	fprintf(fs, "        $('#eth2vmode').removeAttr('disabled');\n");  
    fprintf(fs, "        $('#eth3vmode').removeAttr('disabled');\n");  
    fprintf(fs, "        $('#eth4vmode').removeAttr('disabled');\n");  
    fprintf(fs, "        $('#cpuvmode').removeAttr('disabled');\n");
    fprintf(fs, "	  }else{\n");  
    fprintf(fs, "        $('#eth1vid').attr('disabled', 'disabled');\n");
	fprintf(fs, "   $('#eth1vid').val(1);\n");
    fprintf(fs, "        $('#eth1vmode').attr('disabled', 'disabled');\n"); 
	...
	fprintf(fs, "				</select>\n");
	fprintf(fs, "			</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='cpuvid' size='8'>[范围：1~4094]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH1</td>\n");
	fprintf(fs, "			<td class='diagdata'>\n");
...
void cgiTemplateEdit(char *query, FILE *fs)
{
	...
	uint8_t mymac[6] = {0};
	uint8_t mySid[32] = {0};
	st_dbsCnuDefaultProfile profile;

	char action[IFC_LARGE_LEN];

	memset(&profile, 0, sizeof(st_dbsCnuDefaultProfile));
		
	cgiGetValueByName(query, "profileid", action);
	id = atoi(action);

	printf("id=%d============",id);
	if( CMM_SUCCESS == dbsGetDepro(dbsdev, id, &profile) ){
	...
	fprintf(fs, "<script language='javascript'>\n");	
	fprintf(fs, "function frmLoad(){\n");
	fprintf(fs, "	$('#col_cpuPortSts').val(%d);\n", profile.col_cpuPortSts);
	fprintf(fs, "	hidePortStatusInfo(%s)\n", profile.col_cpuPortSts?"false":"true");
	fprintf(fs, "	$('#eth1portsts').val(%d);\n",  profile.col_eth1sts);
	fprintf(fs, "	$('#eth2portsts').val(%d);\n",  profile.col_eth2sts);
	fprintf(fs, "	$('#eth3portsts').val(%d);\n",  profile.col_eth3sts);
	fprintf(fs, "	$('#eth4portsts').val(%d);\n",  profile.col_eth4sts);	
	...
	fprintf(fs, "	$('#eth3vid').val(%d);\n", profile.col_eth3vid);	
	fprintf(fs, "	$('#eth4vid').val(%d);\n", profile.col_eth4vid);	
	fprintf(fs, "	$('#rxBctrlStatus').val(%d);\n", profile.col_rxLimitSts);
	fprintf(fs, "	$('#txBctrlStatus').val(%d);\n", profile.col_txLimitSts);
	fprintf(fs, "	$('#eth1rxvalue').val(%d);\n", profile.col_eth1rx);
	fprintf(fs, "	$('#eth2rxvalue').val(%d);\n", profile.col_eth2rx);
	fprintf(fs, "	$('#eth3rxvalue').val(%d);\n", profile.col_eth3rx);
	fprintf(fs, "	$('#eth4rxvalue').val(%d);\n", profile.col_eth4rx);
	...
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH1端口接收限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth1rxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH2端口接收限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth2rxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH3端口接收限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth3rxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH4端口接收限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth4rxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>CABLE端口接收限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='cablerxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'>\n");
	...
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH1端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth1txvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH2端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth2txvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH3端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth3txvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>ETH4端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='eth4txvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "		<tr>\n");
	fprintf(fs, "			<td class='diagdata'>CABLE端口发送限速</td>\n");
	fprintf(fs, "			<td class='diagdata'><input type='text' id='cabletxvalue' size='10'>*64Kbits/s [范围：0~1526]</td>\n");
	fprintf(fs, "		</tr>\n");
	fprintf(fs, "	</table>\n");
	fprintf(fs, "</div>\n");	
	fprintf(fs, "<h3>报文抑制</h3>\n");
...
void cgiTemplateView(char *query, FILE *fs) 
{
	...
	fprintf(fs, "function btnRefresh(){\n");
	fprintf(fs, "	location.href = 'previewTemplate.cgi';\n");
	fprintf(fs, "}\n");
	fprintf(fs, "function btnNewCnu(){\n");
	fprintf(fs, "	window.location='editTemplate.cgi';\n");
	fprintf(fs, "}\n");
	...
	fprintf(fs, "<br>\n");
	fprintf(fs, "<table border=0 cellpadding=5 cellspacing=0>\n");
	fprintf(fs, "	<tr><td class='maintitle'><b>模板管理</b></td></tr>\n");
	fprintf(fs, "</table>\n");
	fprintf(fs, "<table border=0 cellpadding=0 cellspacing=0 width='100%'>\n");
	fprintf(fs, "	<tr><td class='mainline' width='100%'></td></tr>\n");
	fprintf(fs, "</table>\n");
	...
	fprintf(fs, "<br><br>\n");	
	fprintf(fs, "<table border=0 cellpadding=0 cellspacing=0>\n<tr>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>模板ID</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=100>VLAN使能</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>ETH1 VLAN</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>ETH2 VLAN</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>ETH3 VLAN</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>ETH4 VLAN</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>下行限速(Kbps)</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=80>上行限速(Kbps)</td>\n");
	fprintf(fs, "<td class='diagret' align='center' width=100>查看编辑</td>\n</tr>\n");
	fprintf(fs, "<tr>\n<td colspan=7>&nbsp;</td>\n</tr>\n");


	//get default system 10 template
	for( i=3; i<=12; i++ )
	{	
		if( CMM_SUCCESS == dbsGetDepro(dbsdev, i, &profile) )
		{
			
				iCount = 0;
				
				fprintf(fs, "<tr>\n");
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.id);
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_vlanSts);					
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_eth1vid);
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_eth2vid);
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_eth3vid);
				...
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_cpuPortRxRate);				
				fprintf(fs, "<td class='clt' align='center'>%d</td>\n", profile.col_cpuPortTxRate);				
				fprintf(fs, "<td class='clt' align='center'><img src='ico_Editor.gif' style='cursor:pointer' onclick='window.location=\"wecTemplateEdit.cmd?profileid=%d\"'>\n", i);
				fprintf(fs, "</td>\n");
				fprintf(fs, "<tr>\n");
		...
		{
			/**/
			fprintf(fs, "<tr>\n<td class='clt' align='center' width=700>** get db profile 系统错误，请重试 **</td>\n<tr>\n");
			strcpy(logmsg, "show template");
			http2dbs_writeOptlog(-1, logmsg);
		}
	}	
	fprintf(fs, "<tr>\n<td colspan=7>&nbsp;</td>\n</tr>\n");
	...
	fprintf(fs, "<p>\n");
	fprintf(fs, "	<button id='refreshBtn'>刷 新</button>\n");
	fprintf(fs, "	<button id='newCnuBtn'>创建模板</button>\n");
	fprintf(fs, "</p>\n");
	fprintf(fs, "</blockquote>\n</body>\n</html>\n");
	fflush(fs);
}
