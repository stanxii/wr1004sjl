...
snmpd_parse_config_trap2sink(const char *word, char *cptr)
{
	...
	{		
		...
		if (create_v2_trap_session(snmpConfig.col_sina, pp, snmpConfig.col_trapcom) == 0) 
		{
			config_perror("cannot create trap2sink\n");
		}
		else
		{
			printf("Starting snmp trap processor	......          [OK]\n");
			printf("trap-server: %s/%s/%s\n", snmpConfig.col_sina, pp, snmpConfig.col_trapcom);
			g_trapSerSts = 1;
		}
	}
}
