...
int main(void)
{
	...
	envptr = (env_t *)malloc(sizeof(env_t));
	if( NULL == envptr )
	{
		printf("\n***mkenv error: cannot malloc memery***\n");
		return -1;
	}

	memset(envptr->data, 0xff, ENV_SIZE);
	p = envptr->data;
	while(strcmp(envtable[i], "end") != 0 )
	{
		strcpy(p, envtable[i]);
		
		p += strlen(envtable[i]);
		envlen += strlen(envtable[i]);
	...
	*p = '\0';
	...
	envlen = sizeof(env_t) - (ENV_SIZE - envlen);
	...
	while(envlen--)
	{
		if( envptr->data[i] == '\0' )
			...
			printf("%c", envptr->data[i]);
		i++;
	}
#else
	printf("\nenvlen = %d\n", envlen);
	if( (fp = fopen(ENV_IMG_FILE, "wb")) < 0 )
	{
		fprintf(stderr, "ERROR: file %s can not be created!\n", ENV_IMG_FILE);
	}
	else
	{
		//fwrite((char *)envptr, 1, envlen, fp);
		fwrite((char *)envptr, 1, sizeof(env_t), fp);
		fclose(fp);
	}
#endif
