...
/* frank 添加的 */
ULONG CLI_KEY_IS_FOR_CLT(const char *cmd)
{
	if( strcmp(cmd, "cur-profile") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "reload-profile") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "aging-time") == 0 )
	{
		return 1;
	}
...
/* frank 添加的 */
ULONG CLI_KEY_IS_FOR_CNU(const char *cmd)
{
	if( strcmp(cmd, "cur-profile") == 0 )
	{
		return 1;
	...
	else if( strcmp(cmd, "rate-limit") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "storm-filter") == 0 )
	{
		return 1;
	}
	...
	else if( strcmp(cmd, "mac-limit") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "qos") == 0 )
	...
	else if( strcmp(cmd, "ar8236-reg") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "ar8236-phy") == 0 )
	{
		return 1;
	}
	else if( strcmp(cmd, "reload-profile") == 0 )
...
ULONG  CLI_ShowHelp( PST_CLI_POINTER pPointer, ULONG  ulCurMode, ULONG  ulStatus)
{
    ...
    if(m_sCliSysModes[ulCurMode].ulMode == ulCurMode)
    {
        ...
        {
        	/* frank 添加的 */
        	if( (ulCurMode>=CTM_IF_1_CLT)&&(ulCurMode<=CTM_IF_4_CLT) )
		{
			IO_Print("\r\n\r\n  Command Word [ interface clt] ");
		}
		else if( (ulCurMode>=CTM_IF_1_1_CNU)&&(ulCurMode<=CTM_IF_1_64_CNU) )
		{
			IO_Print("\r\n\r\n  Command Word [ interface cnu ]");
		}
		else
		{
		...
		if( (ulCurMode>=CTM_IF_1_CLT)&&(ulCurMode<=CTM_IF_4_CLT) )
		{
			IO_Print("\r\n\r\n  Next Command Word [ Interface CLT ] ");
		}
		else if( (ulCurMode>=CTM_IF_1_1_CNU)&&(ulCurMode<=CTM_IF_1_64_CNU) )
		{
			IO_Print("\r\n\r\n  Next Command Word [ Interface CNU ]");
		}
		else
		{
    ...
    while(pSubCmdKey != NULL)
    {
        if (pSubCmdKey->action.ulMode == CTM_GLOBAL)
        {
        	...
        	if( (ulCurMode>=CTM_IF_1_CLT)&&(ulCurMode<=CTM_IF_4_CLT) )
		{
			if( CLI_KEY_IS_FOR_CNU(pSubCmdKey->szName) )
			{
				if( !CLI_KEY_IS_FOR_CLT(pSubCmdKey->szName) )
				{
					pSubCmdKey = pSubCmdKey->pNext;
					continue;
				}
...
ULONG CLI_GetPrevailMode(ULONG mode)
{
	switch(mode)
	{
		case CTM_IF_1_CLT:
...
ULONG CLI_GetCltTidByMode( ULONG mode)
{
	switch(mode)
	{
		case CTM_IF_1_CLT:
...
{
   // return CLI_GetModePrompt(m_stCliTerm.szCurUserName);
    return CLI_GetModePrompt(m_stCliWS.ulCurrentMode);
	
#if 0
    char *promt=(char *)malloc(sizeof(char)*CLI_MAX_MODE_PROMPT);
    if(STB_StrCmp(m_stCliTerm.szCurUserName," admin")==0)
    {
	promt=">>admin>";
    }
    else if(STB_StrCmp(m_stCliTerm.szCurUserName," operator")==0)
    {
	promt=">>operator>";
    }
    else if(STB_StrCmp(m_stCliTerm.szCurUserName," user")==0)
    {
	promt=">>user>";
    }
    return promt;
