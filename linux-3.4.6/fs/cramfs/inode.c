...
//add by frank
#ifndef CRAMFS_NAND_SKIPPING_BADBLOCK
#define CRAMFS_NAND_SKIPPING_BADBLOCK
#endif

#ifdef CRAMFS_NAND_SKIPPING_BADBLOCK
...
static void cramfs_fill_nand(struct super_block *sb) 
{
	...
	if(MAJOR(sb->s_dev) == MTD_BLOCK_MAJOR)
	{
		...
		for(i = 0; i < blocks; i++)
		{
			if (mtd->_block_isbad(mtd, (loff_t)i*mtd->erasesize))
			{
				printk("cramfs skipping bad nand block, offset : 0x%08x\n", (i*mtd->erasesize));
